...
import { BlogApi, ProjectApi, apiReqWithAuth } from 'lib/api';
    ...
    <Stack>
      {top && (
        <Stack direction="row" sx={{ p: 2 }} justifyContent="space-between">
          <Button LinkComponent={Link} href="/admin/project/new" variant="contained" color="primary">
            Create Project
          </Button>
...
const BlogListPage: React.FC<PageProps> = ({ settings }) => {
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      ...
      await apiReqWithAuth({ endpoint: GET_PROJECT(selectedId), method: 'DELETE' });
      setLoading(true);
    } catch (e) {
    ...
    () => [
      ...
      // },
      {
        ...
        Cell: ({ row }: { row: Row<Blog> }) => {
          return (
            <Box component={Link} sx={{ '&:hover': { cursor: 'pointer' } }} href={`/admin/project/${row.original.slug}`}>
              {row.original.title}
            </Box>
          );
        }
        ...
        Cell: ({ row }: { row: Row<Blog> }) => {
          return (
            <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
              <Tooltip title="Edit">
                <Link href={`/admin/project/${row.original.slug}`}>
                  <IconButton>
  ...
  const loadData = async (pageIndex: number, pageSize: number): Promise<void> => {
    ...
    const blogs = await api.getProjects();
  ...
  return (
    <Layout admin>
      <Page title="Projects" settings={settings}>
