...
export default function PlaceholderContent({ type, title, bgImage = true, document }: Props) {
  if (document) {
    return (
      <Stack
        spacing={2}
      ...
      {type !== DropzopType.standard && (
        ...
        >
          <Box sx={{ ml: 1 }} position="relative" height={150} width={150}>
            {bgImage && <Image fill src={`${basePath}/images/upload.svg`} alt="upload palceholder" />}
          </Box>
          {/* <CardMedia component="img" image={UploadCover} sx={{ width: 150 }} /> */}
          <Stack sx={{ p: 3 }} spacing={1}>
            {title ? <Typography variant="h5">{title}</Typography> : <Typography variant="h5">Drag & Drop or Select file</Typography>}
